...
static inline bool read_ul(FILE *stream, uint32_t *value) {
  if (fread((char *)value, sizeof *value, 1, stream) != 1) return false;
  SWAP_VL(*value);
  return true;
}
...
static bool read_meta(FILE *stream) {
  ...
  if (de.tag != MAKE_TAG(0x2, 0x0) || de.vr != E_UL || de.vl != 4) return false;
...
static bool read_item_def(dataset *ds, FILE *stream,
                          const uint32_t itemlen) WUR;
...
/* return 0 upon error, which is an impossible length for an undefined length
 * Sequence */
...
static uint32_t read_encapsulated_pixel_data(dataset *ds, FILE *stream) {
  ...
  do {
    ...
    if (ds->handle_pixel_data_item)
      (ds->handle_pixel_data_item)(ds, stream, de.vl);
    epdlen += de.vl;
  } while (1);
