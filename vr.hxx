 ...
 * - While char[2] seems like the most obvious solution we prefer a slightly more complex
 *   implementation to handle dual VR (eg. OB/OW)
 */
struct value_representation
{
// actual types for VR (PS 3.5)
// do not use scoped enum since I need implicit conversion to int for strict weak ordering:
...
UN = 5133, // Unknown
UR = 5137, // Universal Resource Identifier or Universal Resource Locator (URI/URL)
...
#endif
	friend bool operator == (const value_representation & lhs, const value_representation & rhs) { return lhs.val == rhs.val; }
#if 0
private:
struct vr_impl {
...
std::ostream & operator<<(std::ostream &os, const value_representation::str & s);
